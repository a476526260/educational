<template>
  <div class="container">
    ...
    <el-dialog :visible.sync="dialogFormVisible" width="600px" custom-class="editor-dialog">
      <div slot="title" class="dialog-title">{{editorType}}管理员账号</div>
      <el-form label-position="right" :model="editorData">
        <el-form-item label="账号名：" label-width="100px" required v-if="editorType === '添加'">
          <el-input v-model="editorData.name" size="mini"></el-input>
        </el-form-item>
        <el-form-item label="登录密码：" label-width="100px" required>
...
export default {
  ...
  methods: {
    getTableData () {
      _axios.fetchGet('/admin/v1/admin', {
      ...
      switch (this.editorType) {
        case '添加':
          if (this.editorData.name === '') {
            this.errorTip('请填写管理员名称！');
            return false;
          }

          if (this.editorData.name === '') {
            this.errorTip('请填写登录密码！');
            return false;
          }
          _axios.fetchPost('/admin/v1/admin', {
            account: this.editorData.name,
          ...
          break;
        case '编辑':
          _axios.fetchPut('/admin/v1/admin/' + this.editorID, {
            password: this.editorData.password
          }).then(res => {
            if (res.data.code === 200) {
