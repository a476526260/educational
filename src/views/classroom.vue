<template>
  <div class="container">
    ...
    <el-dialog :visible.sync="dialogFormVisible" width="600px" custom-class="editor-dialog">
      <div slot="title" class="dialog-title">{{editorType}}教室</div>
      <el-form label-position="right" :model="editorData">
        <el-form-item label="教室名称：" label-width="100px" required>
          <el-input v-model="editorData.classroom" size="mini"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    getTableData () {
      _axios.fetchGet('/admin/v1/classroom', {
    ...
    handleEditor(data) {
      ...
      this.editorData.classroom = data.classroom;
    },
    handleDelete(data) {
      this.$confirm('确定删除本条信息吗？', '提示', {
        ...
        .then(res => {
          _axios
            .fetchDel('/admin/v1/classroom/' + data.id)
            .then(res => {
              let result = res.data;
              if (result.code === 200) {
    ...
    editorSubmit() {
      if (this.editorData.classroom === '') {
        this.errorTip('请填写教室名称！');
        return false;
      }
      switch (this.editorType) {
        case '添加':
          _axios.fetchPost('/admin/v1/classroom', {
            classroom: this.editorData.classroom
          }).then(res => {
          ...
          break;
        case '编辑':
          _axios.fetchPut('/admin/v1/classroom/' + this.editorID, {
            classroom: this.editorData.classroom
          }).then(res => {
            if (res.data.code === 200) {
