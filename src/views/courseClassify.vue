<template>
  <div class="container">
    ...
    <el-dialog :visible.sync="dialogFormVisible" width="600px" custom-class="editor-dialog">
      <div slot="title" class="dialog-title">{{editorType}}课程分类</div>
      <el-form label-position="right" :model="editorData">
        <el-form-item label="分类名称：" label-width="100px" required>
          <el-input v-model="editorData.typename" size="mini"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    getTableData () {
      _axios.fetchGet('/admin/v1/coursetype', {
    ...
    handleAdd() {
      ...
      this.editorType = '添加';
      this.editorData.typename = '';
    },
    handleEditor(data) {
      this.dialogFormVisible = true;
      this.editorType = '编辑';
      this.editorID = data.id;
      this.editorData.typename = data.typename;
    },
    handleDelete(data) {
      this.$confirm('确定删除本条信息吗？', '提示', {
        ...
        .then(res => {
          _axios
            .fetchDel('/admin/v1/coursetype/' + data.id)
            .then(res => {
              let result = res.data;
              if (result.code === 200) {
    ...
    editorSubmit() {
      if (this.editorData.typename === '') {
        this.errorTip('请填写课程分类名称名称！');
        return false;
      }
      switch (this.editorType) {
        case '添加':
          _axios.fetchPost('/admin/v1/coursetype', {
            typename: this.editorData.typename
          }).then(res => {
          ...
          break;
        case '编辑':
          _axios.fetchPut('/admin/v1/coursetype/' + this.editorID, {
            typename: this.editorData.typename
          }).then(res => {
            if (res.data.code === 200) {
