<template>
  <div class="container">
    ...
    <el-dialog :visible.sync="dialogFormVisible" width="600px" custom-class="editor-dialog">
      <div slot="title" class="dialog-title">{{editorType}}专业信息</div>
      <el-form label-position="right" :model="editorData">
        <el-form-item label="专业名称：" label-width="100px" required>
          <el-input v-model="editorData.major_name" size="mini"></el-input>
        </el-form-item>
        <el-form-item label="层次：" label-width="100px" required>
          <el-input v-model="editorData.level" size="mini"></el-input>
        </el-form-item>
        <el-form-item label="学制：" label-width="100px" required>
          <el-input v-model="editorData.length_of_schooling" size="mini"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    getTableData () {
      _axios.fetchGet('/admin/v1/major', {
    ...
    handleAdd() {
      ...
      this.editorType = '添加';
      this.editorData.major_name = '';
      this.editorData.level = '';
      this.editorData.length_of_schooling = '';
    },
    handleEditor(data) {
      this.dialogFormVisible = true;
      this.editorType = '编辑';
      this.editorID = data.id;
      this.editorData.major_name = data.major_name;
      this.editorData.level = data.level;
      this.editorData.length_of_schooling = data.length_of_schooling;
    },
    handleDelete(data) {
      this.$confirm('确定删除本条信息吗？', '提示', {
        ...
        .then(res => {
          _axios
            .fetchDel('/admin/v1/major/' + data.id)
            .then(res => {
              let result = res.data;
              if (result.code === 200) {
    ...
    editorSubmit() {
      if (this.editorData.major_name === '') {
        this.errorTip('请填写专业名称！');
        return false;
      }
      ...
      switch (this.editorType) {
        case '添加':
          _axios.fetchPost('/admin/v1/major', {
            major_name: this.editorData.major_name,
            level: this.editorData.level,
            length_of_schooling: this.editorData.length_of_schooling,
          }).then(res => {
          ...
          break;
        case '编辑':
          _axios.fetchPut('/admin/v1/major/' + this.editorID, {
            major_name: this.editorData.major_name,
            level: this.editorData.level,
            length_of_schooling: this.editorData.length_of_schooling,
          }).then(res => {
